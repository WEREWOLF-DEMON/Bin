...
def check_prepaid(response):
    try:
        return "✅ Prepaid" if response.get("prepaid", False) else "❌ Non-Prepaid"
    except:
        return "⚠️ Unknown"
...
def format_response(response):
    try:
        prepaid_status = check_prepaid(response)
...
def send_message(token, chat_id, bins):
    response = bin_lookup(bins)
    ...
    try:
        ...
        s().get(api)
        log_activity(response["bin"], "Message Sent")
    except Exception as e:
...
def check_bin_working(bin):
    response = bin_lookup(bin)
    if "error" in response:
        return {"error": "BIN not working"}
    return response

# Filter working BINs
def filter_working_bins(bins):
    working_bins = []
    for bin in bins:
        response = check_bin_working(bin)
        if "error" not in response:
            working_bins.append(bin)
    return working_bins

# Save working BINs to a file
def save_working_bins_to_file(working_bins):
    with open("working_bins.txt", "a") as file:
        for bin in working_bins:
            file.write(f"{bin}\n")

# Send working BINs to Telegram
def send_working_bins_to_telegram(token, chat_id, working_bins):
    for bin in working_bins:
        send_message(token, chat_id, bin)

# Log working BINs
def log_working_bins(bin):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{now}] Working BIN: {bin}")
